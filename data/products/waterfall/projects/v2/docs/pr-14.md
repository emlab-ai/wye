...
```
  ...
  - choice:memory.consistent-verdicts-in-the-log duplicate, refines and contradicts verdicts are written under the node (verdict: line; contradicts and duplicate also open a contradiction: line); consistent verdicts are kept only in _build/verdicts.json — the replayable judge log with model and prompt hash — and the Inbox row says "checked against n neighbours". `ctx verdicts <id>` and `wf verdicts <id>` print all of them.
