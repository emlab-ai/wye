---
node: pr:pr-rename-project-are-working-form-waterfall
type: pr
...
finished: 2026-09-19T12:59:46.447Z
---
...
question:waterfall.public-repo-data The target repo is public and the repo tracks `data/products/yessensei` (YesSensei's PRD, tech design, source docs — 30 files) next to Waterfall's own knowledge. Push it as is, drop yessensei from the repo first, or make the repo private? Once pushed it is public history. part of pr:pr-rename-project-are-working-form-waterfall #resolved

## Tasks

- [x] task:waterfall.rename-code Rename the package names and every user-facing "Waterfall" to Wye in the code, README, prompts and skills; tests green. part of pr:pr-rename-project-are-working-form-waterfall (session: f92e5b9b59)
- [x] task:waterfall.strip-yessensei Rewrite history to drop data/products/yessensei, keep it on disk and ignored. part of pr:pr-rename-project-are-working-form-waterfall (session: f92e5b9b59)
- [x] task:waterfall.rename-remote Add origin https://github.com/emlab-ai/wye and push main (after question:waterfall.public-repo-data). part of pr:pr-rename-project-are-working-form-waterfall (session: f92e5b9b59)
- [ ] task:waterfall.yessensei-own-repo Give YesSensei's knowledge a home of its own (its repo or a WATERFALLDATA folder) — after the strip it is only on disk, unversioned. part of pr:pr-rename-project-are-working-form-waterfall
- [ ] task:waterfall.rename-slug Move the product slug, folder, URLs and skill names to wye once question:waterfall.rename-slug is answered. part of pr:pr-rename-project-are-working-form-waterfall
- [ ] task:waterfall.rename-folder Rename the checkout ~/Projects/waterfall → ~/Projects/wye and re-run install.sh (skill links, ctx path); the app's cwd and the memory folder follow the path. part of pr:pr-rename-project-are-working-form-waterfall

## Result

Renamed to Wye in code and brand (package names wye/@wye/web/@wye/desktop, app title, rail, slash menu, desktop window, README, agent contract, skills; product title Wye) — wf/ctx CLIs and the product slug data/products/waterfall kept (decision:waterfall.rename-scope). Stripped data/products/yessensei from git history and ignored it (decision:waterfall.strip-yessensei-before-push), then pushed main to https://github.com/emlab-ai/wye. Open: question:waterfall.rename-slug (move slug/folder/URLs/skill names to wye?). Follow-ups: task:waterfall.rename-slug, task:waterfall.rename-folder, task:waterfall.yessensei-own-repo.

Blocks this plan produced:

- added decision:waterfall.rename-scope — Rename to Wye in code and brand; keep the CLIs and the product slug
- added question:waterfall.rename-slug — Should the product slug and folder move too — `data/products/waterfall` → `data/products/wye`, `product:waterf
- added question:waterfall.public-repo-data — The target repo is public and the repo tracks `data/products/yessensei` (YesSensei's PRD, tech design, source 
- added task:waterfall.rename-code — Rename the package names and every user-facing "Waterfall" to Wye in the code, README, prompts and skills;
- added task:waterfall.rename-remote — Add origin https://github.com/emlab-ai/wye and push main (after question:waterfall.public-repo-data).
- added task:waterfall.rename-slug — Move the product slug, folder, URLs and skill names to wye once question:waterfall.rename-slug is answered.
- added task:waterfall.rename-folder — Rename the checkout ~/Projects/waterfall → ~/Projects/wye and re-run install.sh (skill links, ctx path);
- added module:memory-review — Memory — review against 2026 research
- added goal:memory.validated-asks — Every ask is validated against the constraints in force, and every decision becomes memory
- added decision:memory.constraint-packet — A request's first message carries the constraints in force, computed structurally, not found by the agent
- added req:memory.intake-packet — A request reaches the agent with the constraints that govern it
- added decision:memory.bitemporal — Every node can say since when and until when it holds, and what superseded it; current means not ended
- added req:memory.current-by-construction — Superseded, rejected and retired knowledge leaves current retrieval, should move legacy knowledge to archive
- added decision:memory.write-time-verdict — A new decision, requirement, rule or constraint is classified against its neighbours when it is written, before the Inbo
- added req:memory.verdicts — A proposed block arrives in the Inbox with its verdicts
- added decision:memory.constraint-type — A constraint is its own type — a product rule without a code source — and the approved ones are the constitution every a
- added decision:memory.consolidate-sessions — When a session ends, what it decided is extracted from the transcript and diffed against what was written
- added decision:memory.forgetting — Done work leaves default retrieval; nothing is deleted
- added decision:memory.evidence — A decision, requirement, rule or constraint names its evidence
- added decision:memory.shapes — The checks ctx enforces per type are declared on the type, not in the linter
- added decision:memory.benchmark — The 78 existing contradicts edges are the regression set for the verdict pass
- added decision:memory.code-source — Code files are a second source of nodes — a comment whose first token is an id defines that node, with the same grammar 
- added question:memory.code-source.kinds — Which kinds may be defined in code, and may a product widen the list?
- added question:memory.code-source.writes — Does Wye write into a product's code files?
- added task:memory.constraint-packet — Build the constraint packet (op:api.packet, `wf packet --for`) and put it in the first message under "Constrai
- added task:memory.bitemporal-props — Add since, until, superseded-by, by, session, evidence to type:node;
- added task:memory.decision-statuses — Give the 43 status-less decisions a status (approved where the code follows them, superseded where a later one
- added task:memory.constraint-type — Declare type:constraint in the base ontology, move the product's constraints out of _agent.md and the spec's n
- added task:memory.verdict-pass — The write-time verdict pass on new or changed decision / req / rule / constraint blocks:
- added task:memory.benchmark — The hide-one-edge regression over the 78 contradicts edges:
- added task:memory.consolidate — The consolidation run on session done:
- added task:memory.forgetting — Archived-for-retrieval state for done plans and closed sessions;
- added task:memory.shapes — `shapes:` on type cards read by the parser and enforced by ctx check;
- added task:memory.lint-deep — `ctx check --deep`:
- added task:memory.code-source-spike — The comment pass of the parser over source-roots for one language (TypeScript, `//` and `/** */`), read-only, 
- added task:memory.code-drift — With code-defined entities:
- added pr:pr-implement — implement this
...
- changed pr:pr-work-component-data-table-rule-type — Work on component:data-table, rule:type-tables, rule:goals-and-tasks.
- changed pr:pr-work-in-progress-visibility — Work-in-progress visibility — a Plans folder, a plan per request, all work items on the Agents page
...
- added decision:memory.statuses-stay-with-the-person — The decision-status pass adds supersedes links; approving the 60 proposed decisions stays with the person
...
- added decision:waterfall.strip-yessensei-before-push — Strip data/products/yessensei from history before the first push to the public repo
- added task:waterfall.strip-yessensei — Rewrite history to drop data/products/yessensei, keep it on disk and ignored.
- added task:waterfall.yessensei-own-repo — Give YesSensei's knowledge a home of its own (its repo or a WATERFALLDATA folder) — after the strip it is only
- added task:new-687 — subtask
- added bug:new-732 — test
- added task:new-428 — childe task

672 paragraphs added or changed — [per document](/waterfall/sessions/f92e5b9b59/changes)
