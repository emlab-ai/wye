...
```
  ...
  - context:wf2.parse-cache alex: "test and improve performance of editing and saving, do we need to build some indexes?". Measured on the waterfall product (101 documents, 6.6k nodes): a save took 560 ms — `wye build` spawned as a process (a full parse, 330 ms, plus a 7.5 MB pretty-printed graph.json and an unused data.js), `wye check` spawned again, then the watcher rebuilt a second time — and every save re-rendered the page three or four times at 700 ms each (react-markdown rendering the 64 KB document for a reader nobody sees once the editor is up, and a 1.4 MB RSC payload with the whole node index), while the column's requests read 1,152 change-record files (570 ms) and the session list carried every session's block attribution (1.7 MB every 20 s).
