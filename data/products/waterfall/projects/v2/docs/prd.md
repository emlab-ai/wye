...
```
  ...
  - when:wf2.serve waterfall serve starts
...
```
  ...
  - then:wf2.serve.config-print it prints a ready-to-run `claude mcp add` command and a `[mcp_servers.waterfall]` TOML block for Codex, for both the stdio command and the HTTP url
