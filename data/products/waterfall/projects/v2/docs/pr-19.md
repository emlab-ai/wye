...
- id: decision:memory.eval-build-plan
  ...
  context: >
    "Build it" on module:benchmarks was sent twice. The first request's plan, pr:18, holds the whole
    ...
    request opened pr:19 with an empty definition.
  choice: >
    pr:18 is set back to defining and is the plan the build runs from — the four proposed blocks are
    approved in the Inbox or Build is pressed over the hold; pr:19 is cancelled as a duplicate and
