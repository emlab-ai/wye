---
node: plan:plan-still-bad-now-need-click-tag
type: plan
title: still bad!
status: done
owner: unassigned
...
- [x] task:block-select-knowledge req:wf2.ui.block-select and rule:block-select shipped on dev-design; ui-test:block-select on test-design; task:ontology.block-select done under goal:ontology.graph-editor. Part of plan:plan-still-bad-now-need-click-tag. (session: 367dedec3c)
