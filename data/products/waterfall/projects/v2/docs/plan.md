...
- [ ] task:duplicate-user-event Every user message in a chat session is recorded twice in the transcript (a second `user` event 6–15 s after the first, same text, no images key) since at least 13:59 on 2026-09-17, before image support landed — likely two module instances of agent-host pumping the same queue after dev reloads (stale stdout closure calling an old pump); reproduce on a fresh server, then dedupe by queue item id at emit time. Part of req:wf2.sessions.quiet-console.
- [ ] task:image-annotation Annotate images for agents: an Annotate action on an image block opens it as a locked Excalidraw image with shapes, labels and arrows on top; saving exports SVG, a flattened PNG and a text description (regions with positions, arrows from→to, labels) that `wf resolve` appends to the block so an agent reads the annotations and can look at the PNG. Design proposed in session 94ac3cf3e0, awaiting the person's yes. Part of req:wf2.ui.
