...
`WYE_PRODUCT` — names that do not spell the product), and the knowledge itself (product:waterfall, the folder
...
```
  ...
  - choice:waterfall.rename-scope Rename the package names (wye, @wye/web, @wye/desktop), every user-facing "Waterfall" (app title, rail, slash menu groups, desktop window and tray, README, the agent contract, the skills' text) and the git remote. Keep `wf`, `ctx`, WYE_URL, WYE_PRODUCT as they are, keep product:waterfall and data/products/waterfall until question:waterfall.rename-slug is answered.

  - context:waterfall.rename-scope "Rename the project to wye" can reach the package names and the brand, the CLI names, or the knowledge folder and every node id and URL. The last breaks the running app, this session and every link ever written.

  - alternative:waterfall.rename-scope Rename everything at once, including the product slug and the folder — rejected for now: it rewrites node ids and URLs and breaks the live session. Rename only the git remote — rejected: the request says rename the project.

  - consequence:waterfall.rename-scope The app calls itself Wye; the repo pushes to emlab-ai/wye; the knowledge keeps its ids. A second pass can move the slug and folder when approved.

question:waterfall.rename-slug Should the product slug and folder move too — `data/products/waterfall` → `data/products/wye`, `product:waterfall` → `product:wye`, URLs `/waterfall/…` → `/wye/…`, and the `waterfall-*` skill names? It rewrites every link and node reference and the installed skills; the running app and this session live on the old slug. part of pr:13 #open
...
```
  ...
  - choice:waterfall.strip-yessensei-before-push Rewrite the history with git filter-repo to drop data/products/yessensei, keep the folder on disk and ignore it in data/products/.gitignore, then push main. Alex chose this in chat on 2026-09-19.

  - context:waterfall.strip-yessensei-before-push emlab-ai/wye is public; the repo history tracks YesSensei's knowledge (data/products/yessensei) next to Wye's own. Pushing as is publishes it for good.

  - alternative:waterfall.strip-yessensei-before-push Make the repo private and push as is — rejected, the repo is meant to be public. Push as is — rejected, it leaks.

  - consequence:waterfall.strip-yessensei-before-push Local main is rewritten (every commit id changes); yessensei's knowledge is no longer versioned anywhere until it gets a repo of its own (task:waterfall.yessensei-own-repo). Nobody else has a clone, so no one is disrupted.
...
- [x] task:waterfall.rename-code Rename the package names and every user-facing "Waterfall" to Wye in the code, README, prompts and skills; tests green. part of pr:13 (session: f92e5b9b59)
- [x] task:waterfall.strip-yessensei Rewrite history to drop data/products/yessensei, keep it on disk and ignored. part of pr:13 (session: f92e5b9b59)
- [x] task:waterfall.rename-remote Add origin https://github.com/emlab-ai/wye and push main (after question:waterfall.public-repo-data). part of pr:13 (session: f92e5b9b59)
- [ ] task:waterfall.yessensei-own-repo Give YesSensei's knowledge a home of its own (its repo or a WATERFALLDATA folder) — after the strip it is only on disk, unversioned. part of pr:13
- [ ] task:waterfall.rename-slug Move the product slug, folder, URLs and skill names to wye once question:waterfall.rename-slug is answered. part of pr:13
- [ ] task:waterfall.rename-folder Rename the checkout ~/Projects/waterfall → ~/Projects/wye and re-run install.sh (skill links, ctx path); the app's cwd and the memory folder follow the path. part of pr:13
