...
```
  ...
  - choice:waterfall.rename-scope Rename the package names (wye, @wye/web, @wye/desktop), every user-facing "Waterfall" (app title, rail, slash menu groups, desktop window and tray, README, the agent contract, the skills' text) and the git remote. Keep `wf`, `ctx`, WF_URL, WF_PRODUCT as they are, keep product:waterfall and data/products/waterfall until question:waterfall.rename-slug is answered.
