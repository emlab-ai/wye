...
- [x] task:new-441 in session window if you can show size of context, and tokens spent already by agent - show it (session: 53f99bfd98 c8eadd53da, produced: pr:pr-hide-noise-like-tasks-ok-keep module:wf2-dev module:app-documents module:wf2-test pr:pr-told-show-card-additional-blocks-only module:app-agents)
  - [ ] task:new-687 subtask

    bug:new-732 test

- [x] task:new-917 if we have codex or claude process running it must be visible as live/active sessions, and rename sessions to agents /tabl (session: 6ca1fd641c ffab751604, produced: pr:pr-need-more-work-context-panel-proper module:ontology-design module:wf2-dev module:app module:ontology pr:pr-improve-context-window-click-element-would pr:pr-pannel-text-input-bottom-bar-top pr:pr-table-horizontally-scrollable-just-take-100 pr:pr-text-which-sent-agent-beginning-no pr:pr-work-component-data-table-rule-type pr:pr-work-in-progress-visibility module:v2-prs module:wf2 module:waterfall module:bugs module:app-documents module:wf2-test)
...
- [x] task:new-226 make sure that each page is also a node, like any other block, and we can set type for it too, to choose properties (session: 64813dfdab a4dbc39398, produced: pr:pr-told-show-card-additional-blocks-only module:wf2-dev module:app-knowledge module:wf2-test pr:pr-need-more-work-context-panel-proper pr:pr-hide-noise-like-tasks-ok-keep module:app-documents)
