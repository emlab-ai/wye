...
- [x] task:new-917 if we have codex or claude process running it must be visible as live/active sessions, and rename sessions to agents
- [ ] task:new-453 we need to add. a section/block or page which can show us all cards of specific type with filter, i.e. if i want to show all pages, or all manager or all tasks ..
...
- [ ] task:idle-agent-timeout Idle claude/codex processes pile up (six were up for hours today, each holding its context): stop a live-and-idle conversation after a configurable quiet time and let Resume bring it back with --resume. Part of rule:process-is-active.
