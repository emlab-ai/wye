---
node: pr:pr-still-see-fucking-property-text-input
type: pr
...
What the other session is changing (decision:wf2.text-is-first-block on pr:pr-need-more-work-context-panel-proper):
...
_`- [ ] task:` lines, `part of pr:pr-still-see-fucking-property-text-input`; their check state is what is in progress._
...
- added pr:pr-told-show-card-additional-blocks-only — i told you not to show in the card, additional blocks only first one, the reset is in…
...
- added pr:pr-hide-noise-like-tasks-ok-keep — hide noise like this from tasks?
