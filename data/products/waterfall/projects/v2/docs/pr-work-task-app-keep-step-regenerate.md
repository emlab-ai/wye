---
node: pr:pr-work-task-app-keep-step-regenerate
type: pr
...
- id: decision:app.cards-generator
  ...
  part-of: pr:pr-work-task-app-keep-step-regenerate
...
_`- [ ] task:` lines, `part of pr:pr-work-task-app-keep-step-regenerate`; their check state is what is in progress._

![[task:app.keep-in-step]]

- [x] task:app.cards-script scripts/cards.js: read the cards (component, lib, op, page) from the v2 documents and the files under packages/web/src; coverage by file/source/component and by route mapping; `--check` prints the two lists and exits 1 when either is non-empty; `--json`. Part of pr:pr-work-task-app-keep-step-regenerate, part of task:app.keep-in-step. (session: 7a334e57e6)
- [x] task:app.cards-write `--write`: a new card per uncovered file (kind by path, purpose = header comment, status proposed, into the right document and section, yaml row appended inside the list region), re-point renamed files (same basename, unique), retire cards of deleted files in place; op cards' `source:` tightened to the route file. Part of pr:pr-work-task-app-keep-step-regenerate, part of task:app.keep-in-step. (session: 7a334e57e6)
- [x] task:app.cards-test test/cards.js in `npm test` (the check against the real tree) and a fixture test of the mapping and the header extraction; `npm run cards` script. Part of pr:pr-work-task-app-keep-step-regenerate, part of task:app.keep-in-step. (session: 7a334e57e6)
- [x] task:app.cards-run Run `--write` once: 28 new cards, 3 retired, op sources tightened; ctx check green; lib:cards card; commit. Part of pr:pr-work-task-app-keep-step-regenerate, part of task:app.keep-in-step. (session: 7a334e57e6)
...
- changed task:pr-work-req-document-opened-document-opened — Work on req:document-opened-in-the.
