...
- id: decision:memory.eval-build-plan
  title: The benchmarks build runs from pr:pr-build, reopened; plan-build-2 is the same request and is cancelled
  context: >
    "Build it" on module:benchmarks was sent twice. The first request's plan, pr:pr-build, holds the whole
    ...
    request opened pr:pr-build-2 with an empty definition.
  choice: >
    pr:pr-build is set back to defining and is the plan the build runs from — the four proposed blocks are
    approved in the Inbox or Build is pressed over the hold; pr:pr-build-2 is cancelled as a duplicate and
  ...
  affects: [pr:pr-build, pr:pr-build-2]
...
_`- [ ] task:` lines, `part of pr:pr-build-2`; their check state is what is in progress._

- [ ] task:pr-build-2 build it #review (worker: claude-code, session: a95bf7bbe0, produced: module:app module:memory-review pr:pr-build module:benchmarks)
