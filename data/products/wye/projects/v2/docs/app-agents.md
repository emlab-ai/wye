...
```
  ...
  - context:wf2.cli-is-wye The product was renamed Wye (decision:wye.rename-scope) and the CLI kept its old name, wf. The person asked for the rename on 2026-09-20.
...
```
  ...
  - alternative:wf2.transcript-app-links Rewrite the prompt to omit URLs — the agent needs them for `wye resolve` and the session link; a special `wye://` scheme in the desktop — two forms of the same link, and the web would still show localhost; rendering only in the desktop — the web has the same raw URLs.
