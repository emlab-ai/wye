...
node: pr:13
type: pr
title: rename project we are working on form wye to wye and push changes to remove repo…
status: done
owner: unassigned
...
# rename project we are working on form wye to wye and push changes to remove repo…

## Request

> rename project we are working on form wye to wye and push changes to remove repo https://github.com/emlab-ai/wye

## Context

The repo has no git remote; https://github.com/emlab-ai/wye exists, is empty and **public**. The name lives in three
layers: the code (`package.json` names `wye`, `@wye/web`, `@wye/desktop`; brand strings "Waterfall" in
the app shell, slash menu, desktop window, README, prompts/agent-system.md, the skills), the CLIs (`wf`, `ctx`, `WYE_URL`,
`WYE_PRODUCT` — names that do not spell the product), and the knowledge itself (product:wye, the folder
`data/products/wye`, every URL `/wye/…`, node ids `req&#58;wf2.*`). Related: decision:wf2.product-model,
req:wf.self, rule:agent-contract.

## Plan

```yaml
- id: decision:wye.rename-scope
...
```
  ...
  - choice:wye.rename-scope Rename the package names (wye, @wye/web, @wye/desktop), every user-facing "Waterfall" (app title, rail, slash menu groups, desktop window and tray, README, the agent contract, the skills' text) and the git remote. Keep `wf`, `ctx`, WYE_URL, WYE_PRODUCT as they are, keep the old product slug and folder until question:wye.rename-slug is answered.

  - context:wye.rename-scope "Rename the project to wye" can reach the package names and the brand, the CLI names, or the knowledge folder and every node id and URL. The last breaks the running app, this session and every link ever written.

  - alternative:wye.rename-scope Rename everything at once, including the product slug and the folder — rejected for now: it rewrites node ids and URLs and breaks the live session. Rename only the git remote — rejected: the request says rename the project.

  - consequence:wye.rename-scope The app calls itself Wye; the repo pushes to emlab-ai/wye; the knowledge keeps its ids. A second pass can move the slug and folder when approved.

question:wye.rename-slug Should the product slug and folder move too — the old slug → `data/products/wye`, `product:wye`, URLs `/wye/…`, and the `wye-*` skill names? It rewrites every link and node reference and the installed skills; the running app and this session live on the old slug. Answered yes on 2026-09-21: everything moved, the old ids rewritten in place (session:017wTEs8Jy8fzwycEec3ktJC). part of pr:13 #resolved

```yaml
- id: decision:wye.strip-yessensei-before-push
  title: Strip data/products/yessensei from history before the first push to the public repo
  date: 2026-09-19
  status: proposed
  resolves: question:wye.public-repo-data
  part-of: pr:13
```

  - choice:wye.strip-yessensei-before-push Rewrite the history with git filter-repo to drop data/products/yessensei, keep the folder on disk and ignore it in data/products/.gitignore, then push main. Alex chose this in chat on 2026-09-19.

  - context:wye.strip-yessensei-before-push emlab-ai/wye is public; the repo history tracks YesSensei's knowledge (data/products/yessensei) next to Wye's own. Pushing as is publishes it for good.

  - alternative:wye.strip-yessensei-before-push Make the repo private and push as is — rejected, the repo is meant to be public. Push as is — rejected, it leaks.

  - consequence:wye.strip-yessensei-before-push Local main is rewritten (every commit id changes); yessensei's knowledge is no longer versioned anywhere until it gets a repo of its own (task:wye.yessensei-own-repo). Nobody else has a clone, so no one is disrupted.

question:wye.public-repo-data The target repo is public and the repo tracks `data/products/yessensei` (YesSensei's PRD, tech design, source docs — 30 files) next to Waterfall's own knowledge. Push it as is, drop yessensei from the repo first, or make the repo private? Once pushed it is public history. part of pr:13 #resolved

## Tasks

- [x] task:wye.rename-code Rename the package names and every user-facing "Waterfall" to Wye in the code, README, prompts and skills; tests green. part of pr:13 (session: f92e5b9b59)
- [x] task:wye.strip-yessensei Rewrite history to drop data/products/yessensei, keep it on disk and ignored. part of pr:13 (session: f92e5b9b59)
- [x] task:wye.rename-remote Add origin https://github.com/emlab-ai/wye and push main (after question:wye.public-repo-data). part of pr:13 (session: f92e5b9b59)
- [ ] task:wye.yessensei-own-repo Give YesSensei's knowledge a home of its own (its repo or a WATERFALLDATA folder) — after the strip it is only on disk, unversioned. part of pr:13
- [x] task:wye.rename-slug Move the product slug, folder, URLs and skill names to wye once question:wye.rename-slug is answered. part of pr:13 (session: 017wTEs8Jy8fzwycEec3ktJC)
- [x] task:wye.rename-folder Rename the checkout from its old name to ~/Projects/wye and re-run install.sh (skill links, ctx path); the app's cwd and the memory folder follow the path. part of pr:13

## Result

Renamed to Wye in code and brand (package names wye/@wye/web/@wye/desktop, app title, rail, slash menu, desktop window, README, agent contract, skills; product title Wye) — wf/ctx CLIs and the old product slug kept (decision:wye.rename-scope). Stripped data/products/yessensei from git history and ignored it (decision:wye.strip-yessensei-before-push), then pushed main to https://github.com/emlab-ai/wye. Open: question:wye.rename-slug (move slug/folder/URLs/skill names to wye?). Follow-ups: task:wye.rename-slug, task:wye.rename-folder, task:wye.yessensei-own-repo.

Blocks this plan produced:

- added decision:wye.rename-scope — Rename to Wye in code and brand; keep the CLIs and the product slug
- added question:wye.rename-slug — Should the product slug and folder move too — the old slug → `data/products/wye`, `product:wye`, URLs
- added question:wye.public-repo-data — The target repo is public and the repo tracks `data/products/yessensei` (YesSensei's PRD, tech design, source 
- added task:wye.rename-code — Rename the package names and every user-facing "Waterfall" to Wye in the code, README, prompts and skills;
- added task:wye.rename-remote — Add origin https://github.com/emlab-ai/wye and push main (after question:wye.public-repo-data).
- added task:wye.rename-slug — Move the product slug, folder, URLs and skill names to wye once question:wye.rename-slug is answered.
- added task:wye.rename-folder — Rename the checkout to ~/Projects/wye and re-run install.sh (skill links, ctx path);
...
- added task:memory.constraint-packet — Build the constraint packet (op:api.packet, `wye packet --for`) and put it in the first message under "Constrai
...
- added task:memory.shapes — `shapes:` on type cards read by the parser and enforced by wye check;
- added task:memory.lint-deep — `wye check --deep`:
...
- added task:memory.impl.shapes — `shapes:` on type cards read by the parser and enforced by wye check;
- added task:memory.impl.forgetting — archived-for-retrieval:
- added task:memory.impl.decision-statuses — statuses on the status-less decisions in wye's documents.
...
- added task:memory.impl.lint-deep — `wye check --deep` over same-kind pairs sharing a neighbour.
...
- added task:exec.impact-cli — `wye impact <id> --after` and `wye graph impact --semantic --explain`;
...
- changed rule:process-is-active — A conversation whose claude or codex process is up is active whatever its recorded status: `wye session done` m
...
- changed rule:agent-runner — External agents connect through the `wf` CLI (bin/wye.js) against the running web app. `wye agent listen --produ
...
- added contradiction:wye.e0f217651378 — decision:memory.forgetting contradicts req:wf2.contradictions — A requires contradictions to stay open until r
- added task:memory.impl.idle-consolidation — Consolidate a chat that went idle for a day, not only one that ended with `wye session done` (decision:memory.c
...
- added decision:wye.strip-yessensei-before-push — Strip data/products/yessensei from history before the first push to the public repo
- added task:wye.strip-yessensei — Rewrite history to drop data/products/yessensei, keep it on disk and ignored.
- added task:wye.yessensei-own-repo — Give YesSensei's knowledge a home of its own (its repo or a WATERFALLDATA folder) — after the strip it is only
- added task:new-687 — subtask
- added bug:new-732 — test
- added task:new-428 — childe task

672 paragraphs added or changed — [per document](/wye/sessions/f92e5b9b59/changes)
