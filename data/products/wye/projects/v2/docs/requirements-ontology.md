...
- id: question:ontology.q2
  ...
  q: >
    Can a type such as person be defined once and used by yessensei and wye, or is every ontology product-local with only the base ontology shared? The design assumes product-local.
  status: open
...
- id: question:ontology.q3
  ...
  q: >
    Phase 3 creates a block: node for every paragraph. Should that happen for all documents (about 1 500 nodes for wye today) or only when a document opts in with blocks: nodes in its frontmatter?
  status: open
...
```
  ...
  contradiction:wye.a4ab2c5826d6 req:ontology.instance-home duplicates decision:ontology.collection-document — Both describe the same feature: instances collect in a type's collection document (plural-named) created on first instance, with `home:` set as the type's default landing point. #approved (between: req:ontology.instance-home decision:ontology.collection-document, conflict: static, reason: Both describe the same feature: instances collect in a type's collection document  plural-named  created on first instance  with `home:` set as the type's default landing point.)
...
```
  ...
  contradiction:wye.3b8dc8b9d832 req:ontology.comment-home contradicts decision:ontology.uniform-content — A (2026-09-18) says comments are blocks in parent's content; B says comments go to Comments document; later decision reversed the earlier stance. #approved (between: req:ontology.comment-home decision:ontology.uniform-content, conflict: dynamic, reason: A  2026-09-18  says comments are blocks in parent's content; B says comments go to Comments document; later decision reversed the earlier stance.)
