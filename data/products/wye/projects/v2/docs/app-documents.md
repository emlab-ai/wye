...
- id: rule:block-menu
  statement: >
    ...
    alike — has a context menu on right-click, where the pointer is: Comment… (a typed node: the column with its
    Comments box focused; a plain block: a box in place — the comment goes on the block's node `block:<doc>.<hash>`
    in the project's Comments document), Copy and Cut (the block with its children to an in-app clipboard, its
    ...
    as text), Remove. A hover on a tag (350 ms) shows the node's card in place — kind, title, status, its text, the
    properties that say something, open › — fetched once per node; the card can be entered and goes when the pointer
    leaves it.
