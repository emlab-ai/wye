...
- **Templates are stored in the system.** Today page templates are files of the Wye repository (`templates/docs/*.md`) and the picker's list is a constant in the app's code — a person cannot see, edit or add one. Skills went the other way a day earlier: they became documents so a person could read and edit them.
