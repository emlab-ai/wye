...
```
  ...
  - choice:wf2.produced-kept-collapsed Keep it (rule:task-artifacts needs a way from the task to what its sessions produced) but render it after Connected as a bar with a show button, closed by default, with a one-line count so a person knows there is something behind it; its data is fetched only when opened.

  - context:wf2.produced-kept-collapsed On task:new-441 the Produced section (sessions, documents, nodes changed) dwarfed the task itself: a one-line todo item followed by a card of twenty tags. The person asked whether to hide it, and allowed keeping it "fully collapsed and somewhere at the bottom".

  - alternative:wf2.produced-kept-collapsed Drop it from the column and rely on the session page's knowledge strip (loses the task → output path); keep it where it is but folded (still above Content and Connected, the things a person came for); show only when the task is done (a running session's output is what a person watches).

  - consequence:wf2.produced-kept-collapsed Content and Connected come first on every goal and task; the sessions and inbox are not requested until asked; ui-test:node-content gains a task with a session.
...
```
  ...
  - choice:wf2.produced-not-remembered Produced opens for the node it was opened on and is closed again on the next node; no localStorage key.

  - context:wf2.produced-not-remembered Related's show / hide is remembered per browser (rule:related-collapsed) because a person who writes with it open wants it open everywhere. Produced is looked at per task.

  - alternative:wf2.produced-not-remembered Remember it like Related (`wf-produced`) — one click would re-inflate every task column for that browser.

  - consequence:wf2.produced-not-remembered one less bit of state in PeekProvider; the toggle lives in Produced itself.
...
- [x] task:wf2.produced-bottom Move Produced below Connected in the column for a goal or task (PeekPanel NodeView) part of pr:12 (session: c8eadd53da)
- [x] task:wf2.produced-collapsed Produced is a bar with show / hide: closed by default with `n sessions · m documents`, sessions and inbox fetched only while open (Produced.tsx, globals.css) part of pr:12 (session: c8eadd53da)
- [x] task:wf2.produced-test Extend ui-test:node-content: a task with a session shows Produced closed after Connected, no session request went out, "show" loads it part of pr:12 (session: c8eadd53da)
- [x] task:wf2.produced-knowledge Ship req:wf2.ui.produced-collapsed and rule:produced-collapsed, update component:produced, req:wf2.ui.node-page and rule:task-artifacts, test-design part of pr:12 (session: c8eadd53da)
...
49 paragraphs added or changed — [per document](/wye/sessions/c8eadd53da/changes)
