...
verified-against: ~/Projects/wye (uncommitted, 0.1.0)
source-roots: [., lib, bin, viewer, skills, test, schema, templates, skills/wye-context, skills/wye-describe-module]
sources:
  - README.md
  - schema/kinds.yaml
...
| 4 | rule:inventory-gate-shorthand | module:wye purpose ("generic") | the generic parser hardcodes inventory's permission gate ids (view / manage / record-ops) | lib/parse.js:143-146 |
...
| 7 | test:smoke | module:yessensei-pos | the only test reads a fixture from another repository and exits 0 with zero assertions when it is missing; a fresh checkout of wye is "green" without testing anything | test/smoke.js:10 |
...
- id: module:wye
  ...
  edges:
    - module:wye -(edge-to)-> module:claude-code       # skills, Agent tool for explorers, Artifact tool for publishing
    - module:wye -(edge-to)-> module:git               # the graph lives in the repo and is reviewed in PRs
    - module:wye -(edge-to)-> module:yessensei-pos     # first consumer; its inventory.md is the pilot and the smoke-test fixture
```
