...
- id: test:web-lib
  ...
  cases:
    ...
    - runs (the run page): stagesSection is the chain — numbered stages, the one the run is on marked, and the gate out of each (what it asks for, what it starts, who moves it), live for the current one; blockingSection says what holds the criterion back or that only the person is missing; withSection replaces the section the engine owns and adds it when the heading is absent
