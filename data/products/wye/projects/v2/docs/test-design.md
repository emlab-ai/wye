...
- id: test:web-lib
  ...
  cases:
    ...
    - runs (the run page): stagesSection writes a step: card per stage of the workflow — every one of them from the first moment, with the live criterion on the stage the run is on and stepStatus projecting todo / running / ready / done / skipped / blocked from the run; blockingSection says what holds the criterion back or that only the person is missing; withSection replaces the section the engine owns and adds it when the heading is absent
