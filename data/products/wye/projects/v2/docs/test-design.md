...
- id: test:web-lib
  ...
  cases:
    ...
    - runs (the run page): stagesSection marks what is done, where it is and what each stage produced; blockingSection says what holds the criterion back or that only the person is missing; withSection replaces the section the engine owns and adds it when the heading is absent
