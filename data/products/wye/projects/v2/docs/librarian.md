...
```
  ...
  - choice:wf2.run-is-a-page `run-<workflow>-<n>.md` under the project's Workflow runs page, so a run is a node, a page and a row of the runs view at once. The frontmatter is the state the engine reads and writes — workflow, `runs-on`, stage, status, produced, sessions, `auto` (the consecutive automatic advances, so the cap survives a restart), started, finished — and four sections are the engine's: **Asked** (what it was started from, in the person's words), **Stages** (the chain as nodes: a `step:` card per stage of the workflow, written when the run starts so the whole chain is in the graph from the first moment and not only the stage it has reached — each with the stage it came from, the run it is part of, what it `needs` before the next one starts, what it `then` starts, and what it produced; their statuses are a projection of the run's state — todo → running while its own work is out → review once that work has ended and what the criterion still asks for is the person's → ready → done; the card of the stage the run is on carries the one move that would work: **Review** while it is `review`, which opens the Inbox on exactly the nodes its criterion is waiting for — every one of them approvable, answerable or rejectable there — and **Complete** while it is `ready`, the same move as Advance, where the person is reading; setting its status to `done` by hand does the same; each card names the session working on it, read from the firings the engine already records, so it holds for sessions started before any of this), **Blocking** (the current stage's criterion row by row with the ids that hold each one back, and how to move it on: the Advance button at the top of the run's own page, or `wye run advance <id>`), **Log** (entered, ready, advanced, reopened, skipped, blocked — with who) and **Result** when it ends. What a run accumulates — the documents it produced, their names, its sessions — is merged with what its page already says rather than replacing it, so a sweep holding a state read a moment ago cannot drop what a stage wrote since. Written only on a transition, so the page does not rewrite itself into a rebuild loop — a write that would not change the file is skipped, which is what lets every sweep refresh Stages and Blocking without one — and one sweep per run at a time, so two rebuilds landing together cannot write the same line twice. Every document a run produces is a `doc:` page — type:doc, a page that is only a page — and not a `module:`, which is a bounded area of the product: a run's research, PRD, designs and plan have no business in the product's list of modules, and a typed page's card is its frontmatter (rule:page-node-line), so the template's own `## 0.` card goes with it. Each one is a child of the run's own page, so the arc is one branch of the document tree rather than siblings at the top (one made before that, or while the run had no page, is adopted — `part-of` is written only when it has none, so a document moved on purpose stays where it was put). A produced document is recorded on the run as `doc-<name>: <id>` when it is made, because a name cannot be recovered from a slug that slugify truncated — and its slug is built as `<target, cut to 48>-<name>` for the same reason: with the name cut off the end, the second stage of a run wrote into the first one's document. The run's own page carries its strip, compact there because the page below repeats what it would otherwise show (it sits in the page's flow — it is not part of the top bar) — the page a person reads is the page they act on (`runsOnNode` matches a run by its id as well as by what it runs on), and the strip says what Advance would start. The property is `runs-on`, not `on`: type:comment owns the verb `on`, and an inverse is keyed by its name, so a run's target came out as a *comment* on the idea. A run written before run pages is moved to one by the next sweep — the page is written from the card's own state, the card is taken out of the Workflow runs document, and one rebuild sees both, so the id is never defined twice. That duplication is what showed the shape was wrong: the view listed the run and the document's own card rendered it again.
