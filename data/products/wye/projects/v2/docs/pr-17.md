...
> 
> When it is done: `wye node set task:app.trace-requirements --status done`; what you leave open stays as task lines under it.
...
_`- [ ] task:` lines, `part of pr:17`; their check state is what is in progress._
...
- changed task:wf2.answer-rules — rule:card-essence and rule:card-fold updated through `wye impact` part of plan:plan-why-t-type-space-typing-ans
...
- changed task:pr-16 — why i can't type space when typing answer???
- changed pr:16 — why i can't type space when typing answer??? also no answer in details, it must be like…
...
- changed task:exec.impl.wye-turns — the librarian's first turn (explain with tags), the question form along when / then / unless, `wye propose` wri
...
- changed pr:15 — build this one too
...
- added task:memory.eval-truth — `wye eval own --build-truth`:
- added task:memory.eval-judge-set — `eval/judge/labels.jsonl` (50 pairs labelled by the person), `wye eval judge --agreement` (Cohen's κ), rerun on
...
- added task:memory.eval-compare — `wye eval compare`:
- added task:memory.eval-public — `eval/public`:
- added task:memory.eval-page — page:web/eval and op:api.eval:

268 paragraphs added or changed — [per document](/wye/sessions/b9b458ace4/changes)
