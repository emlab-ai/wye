...
function verdictLines(v, { product = 'x' } = {}) {
    ...
    if (v.kind === 'contradicts' || v.kind === 'duplicate') lines.push(`contradiction:${product}.${v.key} ${v.b} ${v.kind === 'duplicate' ? 'duplicates' : 'contradicts'} ${v.a} — ${v.reason} #open (between: ${v.b} ${v.a}, conflict: ${v.conflict || (v.kind === 'duplicate' ? 'static' : 'static')}, reason: ${v.reason.replace(/[(),]/g, ' ')})`);
    return lines;
}

module.exports = { judgePairs, nodeText, buildPrompt, parseAnswer, promptHash, pairKey, verdictLines, DEFAULT_MODEL, PROMPT_VERSION };
