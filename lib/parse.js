...
const KINDS = ['req', 'rule', 'entity', 'value', 'state', 'op', 'page', 'action', 'gate', 'flag', 'test', 'ui-test', 'module', 'product', 'tool', 'setting', 'field', 'drift', 'question', 'decision'];
// Short kind aliases for fast typing in prose (et:order → entity:order). Normalised by cleanId.
const ALIASES = { et: 'entity', rq: 'req', rl: 'rule', pg: 'page', st: 'state', dc: 'decision', qn: 'question', vl: 'value', ac: 'action', gt: 'gate', fl: 'flag' };
...
const STATUS_TAG = /(?:^|\s)#(proposed|approved|shipped|unverified|api-only|deprecated|question|drift|done|in-progress|blocked)\b/g;
function idsIn(text) {
...
function parseFiles(files) {
    ...
    for (const file of files) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const pm = l.match(new RegExp('^(\\s*(?:[-*+]|\\d+[.)])\\s+)?(' + ID_RE.source.replace(/\\b/, '') + ')(?=\\s)\\s+(\\S.*)$'));
            const paraStart = !(lines[i - 1] || '').trim() || /^\s*([-*+]|\d+[.)])\s/.test(l);
            if (pm && paraStart && !/^\s*\|/.test(l) && !/^#/.test(l) && !/-\(/.test(l)) {
                const id = cleanId(pm[2]); let text = pm[pm.length - 1];
                let j = i + 1;
