...
const STATUS_TAG = /(?:^|\s)#(proposed|approved|shipped|unverified|api-only|deprecated|question|drift|done|in-progress|blocked|open|todo|non-goal|partial|active|draft|complete|on-track|at-risk|off-track|paused|resolved|rejected|superseded|retired|dismissed)\b/g;
function idsIn(text) {
...
function collectTypes(files) {
    ...
    for (const file of files) {
        ...
        for (const c of yamlChunks(fs.readFileSync(file, 'utf8').split('\n'))) {
            ...
            const t = { id: c.id, slug: c.id.slice(5), extends: null, open: false, purpose: '', home: '', ownProps: [], ownShapes: [], file: rel, line: c.line };
    ...
    if (!types.has('type:node')) types.set('type:node', { id: 'type:node', slug: 'node', extends: null, open: true, purpose: 'the root type', home: '', ownProps: [], ownShapes: [], file: '', line: 0 });
...
function parseWith(files, ontology, kinds) {
    ...
    // ---- forgetting (decision:memory.forgetting): a plan document that ended (status done / failed / cancelled, or
    ...
    for (const m of modules) {
        const page = nodes.get(m.id); if (!page || page.kind !== 'plan') continue;
    ...
    const typeList = [...types.values()].map(t => ({ id: t.id, slug: t.slug, extends: t.extends, chain: t.chain, open: t.open, purpose: t.purpose, home: t.home, props: t.props, shapes: t.shapes, file: t.file, line: t.line }));
