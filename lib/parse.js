...
const STATUS_TAG = /(?:^|\s)#(proposed|approved|shipped|unverified|api-only|deprecated|question|drift|done|in-progress|blocked|open|todo|non-goal|partial|active|draft|complete|on-track|at-risk|off-track|paused|resolved|rejected)\b/g;
function idsIn(text) {
...
// enum [a, b], ref <type>, list of <type|scalar>.
...
function yamlChunks(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
        const l = lines[i];
        if (/^\s*```/.test(l)) {
            if (!inYaml) { inYaml = /^```ya?ml/.test(l); buf = []; start = i + 1; if (!inYaml) { let j = i + 1; while (j < lines.length && !/^\s*```/.test(lines[j])) j++; i = j; } }
...
function collectTypes(files) {
    ...
    for (const file of files) {
        ...
        for (const c of yamlChunks(fs.readFileSync(file, 'utf8').split('\n'))) {
            ...
            const t = { id: c.id, slug: c.id.slice(5), extends: null, open: false, purpose: '', home: '', ownProps: [], file: rel, line: c.line };
    ...
    if (!types.has('type:node')) types.set('type:node', { id: 'type:node', slug: 'node', extends: null, open: true, purpose: 'the root type', home: '', ownProps: [], file: '', line: 0 });
...
function parseWith(files, ontology, kinds) {
    ...
    for (const file of files) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const pm = l.match(new RegExp('^(\\s*(?:[-*+]|\\d+[.)])\\s+(?:\\[( |x|X)\\]\\s+)?)?(' + CUR_ID_RE.source.replace(/\\b/, '') + ')(?=\\s)\\s+(\\S.*)$'));
            ...
            if (pm && paraStart && !/^\s*\|/.test(l) && !/^#/.test(l) && !/-\(/.test(l)) {
                ...
                if (!nodes.get(id) || !nodes.get(id).defined) {
                    ...
                    const extra = []; const g = text.match(/\s*\(([A-Za-z][A-Za-z0-9_-]*:\s*[^()]*?(?:,\s*[A-Za-z][A-Za-z0-9_-]*:\s*[^()]*?)*)\)\s*$/);
        ...
        if (moduleId) {
            ...
            let para = [], start = 0, fence = false, fenceStart = 0, items = [];   // items: [{indent, id}] of the current list
            section = ''; subsection = '';
            const flush = () => {
                if (!para.length) { items = []; return; }
                const text = para.join('\n');
                const allItems = para.every(l => /^\s*(?:[-*+]|\d+[.)])\s/.test(l));
                if (/^\s*(```|~~~)/.test(para[0])) {
                    const ids = cards.get(fenceStart) || [];
                    if (ids.length) for (const c of ids) edge(parentOf(), c, 'has'); else blockNode(text, start, parentOf());
                } else if (!allItems) {
                    if (named.has(start)) edge(parentOf(), named.get(start), 'has'); else blockNode(text, start, parentOf());
                }
                para = []; items = [];
            };
            for (let i = bodyStart; i < lines.length; i++) {
                const l = lines[i];
                if (/^\s*(```|~~~)/.test(l)) { if (!fence) { flush(); fenceStart = i + 1; } fence = !fence; }
    ...
    const typeList = [...types.values()].map(t => ({ id: t.id, slug: t.slug, extends: t.extends, chain: t.chain, open: t.open, purpose: t.purpose, home: t.home, props: t.props, file: t.file, line: t.line }));
