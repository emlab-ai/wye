...
function proseRefs(text) {
    const out = [];
    const linkRe = /\[([^\]]+)\]\(((?!(?:https?:|mailto:|[a-z-]+:\/\/))[a-z-]+:[A-Za-z0-9_./#\-]+)\)/g; let m;
    const covered = [];
    ...
    const urlRe = /\[([^\]]+)\]\((?:https?:|mailto:|[a-z-]+:\/\/)[^)]*\)/gi;
    while ((m = urlRe.exec(text))) covered.push([m.index, m.index + m[0].length]);
...
function collectTypes(files, opts = {}) {
    ...
    for (const { file, chunks } of read) {
        ...
        for (const c of chunks) {
            ...
            const t = { id: c.id, slug: c.id.slice(5), extends: null, open: false, purpose: '', home: '', plural: '', statuses: [], nestsIn: [], ownProps: [], ownShapes: [], file: rel, line: c.line };
            ...
            for (let i = 0; i < bl.length; i++) {
                ...
                else if ((m = l.match(/^statuses:\s*\[(.*)\]\s*$/))) t.statuses = m[1].split(',').map(x => x.trim()).filter(Boolean); // the type's status vocabulary — schema/kinds.yaml is generated from it
                // the kinds this type may sit under (decision:ontology.a-type-can-be-nested-only): a when belongs to a
                // req, never to a page of its own, so the app offers it under one and nowhere else. `node` means any.
                else if ((m = l.match(/^nests-in:\s*(?:\[(.*)\]|(\S+))\s*$/))) t.nestsIn = (m[1] ?? m[2] ?? '').split(',').map(x => x.trim()).filter(Boolean);
...
function parseWith(files, ontology, kinds, opts = {}) {
    ...
    // the inverse name of a ref/list property, written on the other side (`satisfies: [req:x]` on a decision), is the
    // same edge seen from its target: req:x -(satisfied-by)-> the decision. A name that is itself a property is not one.
    const propNames = new Set([...types.values()].flatMap(t => t.props.map(p => p.name)));
    const reverseKeys = {};
    for (const t of types.values()) for (const p of t.props) if (p.ref && p.inverse && !propNames.has(p.inverse) && !EDGE_KEYS[p.inverse]) reverseKeys[p.inverse] = p.name;
    // `edge` is the caller's: inside the per-file pass it is the local recorder, so a replay from the parse cache
    // repeats these edges too (decision:wf2.parse-cache). Taking the outer one would write edges that no file owns,
    // and they would disappear the moment that file replayed instead of being parsed.
    const keyEdges = (edge, id, key, targets) => {
        const verb = EDGE_KEYS[key] || propVerb(id, key);
        if (verb) { for (const t of targets) if (t !== id) edge(id, t, verb); return true; }
        const rv = reverseKeys[key];
        if (rv) { for (const t of targets) if (t !== id) edge(t, id, rv); return true; }
        return false;
    };
    const node = (id, props = {}) => {
    ...
    for (const file of files) {
        ...
        if (moduleId) {
            ...
            for (const fl of fm[1].split('\n')) { const km = fl.match(/^([a-z-]+):\s*(.+)$/); if (!km) continue; keyEdges(edge, moduleId, km[1], idsIn(km[2])); }
        }
        ...
        const flushYaml = (startLine) => {
            ...
            for (const c of chunks) {
                ...
                for (const l of body.split('\n')) {
                    ...
                    if (!key.startsWith('actions') && reverseKeys[key] && !EDGE_KEYS[key] && !propVerb(c.id, key)) { keyEdges(edge, c.id, key, idsIn(l.replace(/^\s*id:.*$/, ''))); continue; }
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (pm && paraStart && !/^\s*\|/.test(l) && !/^#/.test(l) && !/-\(/.test(l)) {
                ...
                if (!nodes.get(id) || !nodes.get(id).defined) {
                    ...
                    for (const ex of extra) keyEdges(edge, id, ex.split(':')[0], idsIn(ex));
                }
                i = j - 1; continue;
            }
        }
        ...
        if (moduleId) {
            ...
            const blockNode = (text, line, parent) => {
                ...
                edge(parent, id, 'has');
                if (!/^\s*(```|~~~)/.test(text)) {
                    let lm; const lre = /\[([^\]]+)\]\(((?!(?:https?:|mailto:|[a-z-]+:\/\/))[a-z-]+:[A-Za-z0-9_./#\-]+)\)/g;
                    while ((lm = lre.exec(text))) {
                        const to = cleanId(lm[2]);
                        edge(id, to, 'related-to');
                        // a passage someone needed to point at (type:text) is defined by the link that names it: the
                        // words are its title and it lives where they are, so a comment or a question can sit on it
                        // without the document carrying a card for every phrase
                        if (to.startsWith('text:')) node(to, { defined: true, title: lm[1].replace(/\s+/g, ' ').trim().slice(0, 110), body: 'text: ' + lm[1].trim(), section, subsection, file: rel, line, form: 'span' });
                    }
                }
                return id;
            };
            ...
            for (let i = bodyStart; i < lines.length; i++) {
                ...
                if (im) {
                    ...
                    // a named paragraph with items right under it (no blank line — a req's when / then / unless) is
                    // their container already, not only once the paragraph is flushed at the blank line after them
                    if (named.has(start) && !/^\s*(?:[-*+]|\d+[.)])\s/.test(para[0]) && indentOf(para[0]) < indent && !open.some(o => o.id === named.get(start))) {
                        edge(parentFor(indentOf(para[0])), named.get(start), 'has'); open.push({ indent: indentOf(para[0]), id: named.get(start) });
                    }
                    const parent = parentFor(indent);
                    let id;
    ...
    // ---- parts (decision:wf2.parts-are-content): a node's text lives in child blocks — when / then / unless,
    // context / choice / alternative / consequence, statement / scope / rationale / note. Readers that look for the
    // old keys (the constraint packet, the constitution, the judge, search) find them: each part's text is read
    // back into the body as a generated key when the card has none, and `partKeys` says which keys were generated
    // so the app's cards never show a part twice.
    {
        const PART_KEY = { when: 'when', then: 'then', unless: 'unless', context: 'context', choice: 'choice', alternative: 'alternatives', consequence: 'consequences', statement: 'statement', scope: 'scope', rationale: 'rationale', note: 'note' };
        const kids = new Map();
        for (const e of edges) if (e.verb === 'has' && PART_KEY[e.to.split(':')[0]]) { if (!kids.has(e.from)) kids.set(e.from, []); kids.get(e.from).push(e.to); }
        for (const [pid, ids] of kids) {
            const n = nodes.get(pid); if (!n || !n.defined) continue;
            for (const cid of ids) {
                const c = nodes.get(cid); if (!c || !c.defined) continue;
                const key = PART_KEY[cid.split(':')[0]];
                const text = (c.body.match(/^text:[ \t]*(.*)$/m) || [])[1]?.trim() || c.title || '';
                if (!text || new RegExp('^' + key + ':', 'm').test(n.body || '')) continue;
                n.body = (n.body ? n.body + '\n' : '') + key + ': ' + text.replace(/\s*\n\s*/g, ' ');
                (n.partKeys = n.partKeys || []).push(key);
            }
            // a constraint or rule with no title of its own reads as its statement, as it did when the key was on the card
            if (!n.title && /^statement:/m.test(n.body)) n.title = n.body.match(/^statement:[ \t]*(.*)$/m)[1].slice(0, 110);
        }
    }
    const seen = new Set();
    ...
    const typeList = [...types.values()].map(t => ({ id: t.id, slug: t.slug, extends: t.extends, chain: t.chain, open: t.open, purpose: t.purpose, home: t.home, plural: t.plural || '', statuses: t.statuses || [], nestsIn: t.nestsIn || [], props: t.props, shapes: t.shapes, file: t.file, line: t.line }));
