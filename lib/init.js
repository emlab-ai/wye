...
- id: ${goal}
  ...
  description: >
    Each module's page holds its requirements (free text, in the person's words), each requirement is
...
`, made);
  ...
  // Product
  writeOnce(path.join(docs, 'product.md'), fm(ids.product, 'Product — what it must do', { parent: rootNode, order: 10 }) + `# Product — what it must do\n\nWhat ${title} must do, in the person's words: who it is for and the jobs (Users and jobs), the requirements by module (free text, in the person's words), the constitution. How it is done is on the Systems pages.\n`, made);
  ...
  const tasks = [
    ...areas.map(a => `- [ ] task:${J}.describe.${a.slug} Describe ${a.title}: read the code under ${a.dir}, write its requirements on module:${J}-req-${a.slug} in the person's words (free text under a title), map each to the lib, component and op cards on module:${J}-${a.slug} that deliver it (file + what that code does), the rules the code enforces with their source, the tests that verify, and a question where the code is unclear. Part of ${goal}. #ready`),
