...
class Graph {
    ...
    check({ repo = process.cwd(), strict = false } = {}) {
        ...
        for (const n of this.data.nodes) {
            ...
            // a type that may only nest (decision:ontology.a-type-can-be-nested-only): a when belongs to a requirement,
            // a context to a decision. One written on its own says nothing — the ontology names what may hold it.
            if (n.defined) {
                const nt = this.typeOf(n.id);
                const into = (nt && nt.nestsIn) || [];
                if (into.length && !into.includes('node')) {
                    const holder = (this.inc.get(n.id) || []).find(e => e.verb === 'has' && into.includes(e.from.split(':')[0]));
                    if (!holder) warnings.push(`${n.id}: a ${n.kind} belongs under a ${into.join(' or a ')} (${nt.id}: nests-in)`);
                }
            }
