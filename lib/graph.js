...
class Graph {
    ...
    // ended nodes (isCurrent) leave search and the packet unless `all` or `asOf` asks for them; `hidden` counts them
    current(nodes, { all = false, asOf = null } = {}) {
        if (all) return { nodes, hidden: 0 };
        const out = nodes.filter(n => isCurrent(n, asOf));
    ...
    packet(task, { budget = 6000, seeds = 6, all = false, asOf = null } = {}) {
        ...
        const parts = [`# Context packet: ${task}\n`, `seeds: ${hits.join(', ')}${cur.hidden ? `  (${cur.hidden} superseded / retired hidden — --all shows them)` : ''}\n`];
...
module.exports = { Graph, sourcesOf, isCurrent, ENDED, PACKET_VERBS, PACKET_KINDS };
