    ...
    check({ repo = process.cwd(), strict = false } = {}) {
        ...
        for (const n of this.data.nodes) {
            ...
            if (n.kind === 'req' && n.defined) {
                if (!outV.includes('satisfied-by') && !outV.includes('see') && !['proposed', 'question'].includes(n.status)) errors.push(`${n.id}: requirement has no satisfied-by (status ${n.status || 'shipped'})`);
            ...
            if (n.kind === 'rule' && n.defined && !/^source:/m.test(n.body)) errors.push(`${n.id}: rule has no source`);
