...
- `question:` and `decision:` become first-class yaml nodes with template sections (§11 questions already use
  `- id: question:…` in `wye.md`; the template and pilot are converted). A `decision:` node is the ADR form:
...
packages/server   `wye serve`: repo registry, chokidar watcher, one in-memory Graph per project, SQLite,
...
docs/context-graph/wye.md   wye's own graph; v2 reqs enter as proposed and flip to shipped with tests.
```
...
prints ready-to-paste config on start: a `claude mcp add` command and a `[mcp_servers.wye]` block for
...
- Waterfall's own graph: every v2 requirement enters `wye.md` as `proposed` and flips to `shipped` with a
...
- Per-node files (drift row 1 in `wye.md`) remain deferred; the writer keeps that door open because the
