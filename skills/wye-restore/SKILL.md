---
name: wye-restore
description: Restore and continue a Wye agent session by id — /wye-restore <session-id> [--product <p>]. Loads the session's instruction, referenced context, log and result, then continues the work from where the previous agent stopped, logging progress back to the same session.
---

# /wye-restore <session-id>
...
Use the wye-agent skill for how to read and write the product's documents.
