...
// (task:new-917). Polls while anything is active. A row opens the conversation in the right column.
...
export function SessionList({ product, initial, initialRunners }: { product: string; initial: Session[]; initialRunners: Runner[] }) {
  ...
  useEffect(() => {
    ...
    const load = async () => { const r = await fetch(`/api/${product}/sessions`); if (!r.ok || !live) return; const j = await r.json(); setSessions(j.sessions); setRunners(j.runners ?? []); timer = setTimeout(load, j.sessions.some(isActive) || (j.runners ?? []).length ? 4000 : 15000); };
  ...
  return (
    <div className="sessions">
      <div className="runners">
        <h4>Agents <span className="muted">{working} working · {idle} live and idle · {runners.length} runner{runners.length === 1 ? '' : 's'} online</span><button className="mini" style={{ marginLeft: 'auto' }} onClick={() => requestSend({})}>+ New conversation</button></h4>
        {runners.length
