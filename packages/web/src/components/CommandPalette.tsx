...
// document, the node under the cursor) travels along as context; images pasted or dropped into the box go with
// the request as the session's files (req:wf2.ui.palette-images).
export function CommandPalette() {
  ...
  const run = async () => {
    ...
    const r = await fetch(`/api/${product}/sessions`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ agent, instruction, refs, source, mode: 'chat', cwd: cwd.trim(), plan, images: attach.images }) });
    ...
    setText(''); attach.clear(); setOn(false); open(`session:${j.id}`);
  };
  return createPortal(
    <div className="modal-back palette-back" onMouseDown={e => { if (e.target === e.currentTarget) setOn(false); }}>
      <div className="modal palette" role="dialog" aria-label="Command" onDragOver={attach.onDragOver} onDrop={attach.onDrop}>
        ...
        <div className="palette-row">
          ...
          <button className="palette-go" onClick={run} disabled={(!text.trim() && !attach.images.length) || busy}>{busy ? 'Starting…' : plan ? 'Plan & build ↵' : 'Run ↵'}</button>
        </div>
