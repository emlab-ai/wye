...
const GROUPS: { key: WorkGroupBy; label: string }[] = [{ key: 'status', label: 'status' }, { key: 'state', label: 'state' }, { key: 'goal', label: 'goal' }, { key: 'plan', label: 'plan' }, { key: 'document', label: 'document' }, { key: 'worker', label: 'worker' }];
...
export function WorkList({ product, items, people }: { product: string; items: WorkItem[]; people: string[] }) {
  ...
  const f: WorkFilter = { q: sp.get('q') ?? '', status: sp.get('status') ?? '', state: sp.get('state') ?? '', worker: sp.get('worker') ?? '', goal: sp.get('goal') ?? '', plan: sp.get('plan') ?? '', doc: sp.get('doc') ?? '', done: sp.get('done') === '1', mine: sp.get('mine') === '1' ? (me || '—') : '' };
  ...
  const visible = useMemo(() => filterWork(items, f), [items, f.q, f.status, f.state, f.worker, f.goal, f.plan, f.doc, f.done, f.mine]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const label = (k: string) => k.startsWith('—') ? k : group === 'goal' || group === 'plan' ? k.replace(/^(goal|plan):/, '') : k;
  const Row = ({ r, depth }: { r: WorkItem; depth: number }) => (
    <>
      <div className={`trow wrow ${r.id === openId ? 'on' : ''} ${r.status === 'done' ? 'done' : ''} st-${r.state}`} style={{ '--depth': depth } as React.CSSProperties} onClick={() => open(r.id)} role="row" data-task={r.id}>
        ...
        <div className="tcell tgoal">{r.plan && !r.requestTask ? <button className="tparent" onClick={e => { e.stopPropagation(); open(r.plan!.id); }} title={r.plan.id}>{plain(r.plan.title)}</button> : r.partOf[0] ? <button className="tparent" onClick={e => { e.stopPropagation(); open(r.partOf[0]); }} title={r.partOf[0]}>{r.partOf[0].replace(/^[a-z-]+:/, '')}</button> : null}</div>
        ...
        <div className="tcell tdoc">
          ...
          <button className="tsend" title="Assign to a worker" onClick={e => { e.stopPropagation(); setAssigning(r); }} disabled={r.status === 'done'}>assign</button>
  ...
  return (
    <div className="track work">
      <div className="track-tools">
        ...
        {(f.goal || f.plan || f.doc) && <div className="chips"><span className="chips-label">in</span>{f.goal && <button className="chip on" onClick={() => set({ goal: null })}>{f.goal} ×</button>}{f.plan && <button className="chip on" onClick={() => set({ plan: null })}>{f.plan} ×</button>}{f.doc && <button className="chip on" onClick={() => set({ doc: null })}>{f.doc} ×</button>}</div>}
      </div>
      <div className="ttable" role="table">
        ...
        {groups.map(([k, rs]) => (
          <div key={k} className="tgroup">
            <div className="tgroup-head" onClick={() => toggle('g:' + k)}><span className="tchev">{collapsed.has('g:' + k) ? '▸' : '▾'}</span><span className="ttitle">{label(k)}</span>{(group === 'goal' || group === 'plan') && !k.startsWith('—') && <button className="linkish small" onClick={e => { e.stopPropagation(); open(k); }}>open</button>}<small className="muted">{rs.length}</small></div>
            {!collapsed.has('g:' + k) && rs.map(r => <Row key={r.id} r={r} depth={0} />)}
          </div>))}
        {!visible.length && <p className="muted" style={{ padding: 16 }}>No work matches.</p>}
      </div>
      {assigning && <Assign product={product} item={assigning} people={people} me={me} onClose={() => setAssigning(null)} onDone={s => { setAssigning(null); if (s) open(`session:${s}`); else router.refresh(); }} />}
    </div>
  );
}
