    ...
    <>
      <div className={`trow wrow ${r.id === openId ? 'on' : ''} ${r.status === 'done' ? 'done' : ''} st-${r.state}`} style={{ '--depth': depth } as React.CSSProperties} onClick={() => open(r.id)} role="row" data-task={r.id}>
        ...
        <div className="tcell tdoc">
          ...
          {r.requestTask && r.plan && r.doc && r.state !== 'working' && r.state !== 'queued' && <button className="tsend" title="Assign the request with the plan's Definition as context" onClick={e => { e.stopPropagation(); setBuilding(`${product}/${r.doc!.project}/${r.doc!.slug}`); setAssigning(r); }} disabled={r.status === 'done'}>build</button>}
