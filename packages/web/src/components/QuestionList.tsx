      ...
      <ul className="qlist">
        {shown.map(r => (
          <li key={r.key} className={`qrow ${isOpen(r) ? '' : 'done'}`} onClick={() => { if (r.id) open(r.id); else if (r.node) open(r.node); else if (r.href) location.href = r.href; }}>
            ...
            {isOpen(r) && <p className="muted qhint">answer it with a decision block next to it, then resolve it in the <a href={`/${product}/inbox`} onClick={e => e.stopPropagation()}>inbox</a></p>}
          </li>
        ))}
      </ul>
    </div>
