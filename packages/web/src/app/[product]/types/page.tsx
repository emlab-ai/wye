  ...
  return (
    <div className="page">
      ...
      <section className="kind-section"><h2>{scope.product.meta.title}&apos;s types <span className="muted">{own.length}</span></h2>
        {own.length ? <TypeRows product={product} rows={own.map(row)} /> : <p className="muted">None yet. Add a <code>type:</code> card to a document — see <Link href={`/${product}/types/node`}>type:node</Link> for the form.</p>}</section>
