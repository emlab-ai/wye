...
import { appendChunk, hashOf, insertYamlAfterSegment, lint, patchFrontmatter, rebuild, replaceChunk, replaceSegment, writeAtomic, type WriteResult } from '@/lib/write';
...
export async function PUT(req: Request, { params }: { params: Promise<{ project: string; slug: string }> }) {
  ...
  return NextResponse.json({ ok: true, rebuilt: built.code === 0, build: built.output.trim(), lintOk: checked.code === 0, lintErrors: errors, hashes });
}
