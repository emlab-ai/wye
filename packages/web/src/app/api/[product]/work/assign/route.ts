...
export async function POST(req: Request, { params }: { params: Promise<{ product: string }> }) {
  ...
  const body = (await req.json()) as { id?: string; worker?: string; note?: string; plan?: boolean; cwd?: string; force?: boolean; agent?: string; by?: string; build?: string };
  if (!body.id || !body.worker) return NextResponse.json({ error: 'invalid', message: 'id and worker required' }, { status: 422 });
  const r = await assignTask(scope, body.id, { worker: body.worker, note: body.note, plan: body.plan, cwd: body.cwd, force: body.force, agent: body.agent, by: body.by, build: body.build, wfUrl: new URL(req.url).origin });
