...
import { getFrontmatter, setFrontmatter } from '@/lib/plan-doc';
...
// status and Definition state (n blocks, k agreed, j open, contradicted, missing); PATCH { ref, status } sets the status.
export async function GET(req: Request, { params }: { params: Promise<{ product: string }> }) {
  ...
  return NextResponse.json({ ref, node: `plan:${plan.slug}`, status: getFrontmatter(plan.md, 'status') ?? '', role: getFrontmatter(plan.md, 'role') ?? 'worker', task: getFrontmatter(plan.md, 'task') ?? null, session: getFrontmatter(plan.md, 'session') ?? '', definition: d }, { headers: { 'cache-control': 'no-store' } });
}
