...
// holds it. Pure: the rows are computed from the graph (task lines anywhere in the documents, the plans they sit on,
...
export type WorkSession = { id: string; status: SessionStatus; agent: string; refs: string[]; createdAt: string; updatedAt?: string; finishedAt?: string; result?: string; planDoc?: string; live?: boolean; busy?: boolean; artifacts?: { blocks?: { id: string }[] } };
...
export type WorkItem = {
  ...
  plan?: { id: string; slug: string; title: string; status: string; task?: string }; // the plan document the task is on; `task` the one it was assigned for
...
export const PLAN_TASK = /^task:(plan-[a-z0-9-]+)$/;
...
export function workItems(g: GraphData, idx: GraphIndex, sessions: WorkSession[]): WorkItem[] {
  ...
  const plansByFile = new Map(g.nodes.filter(n => n.kind === 'plan' && n.defined).map(n => [n.file, n]));
  ...
  for (const n of tasks) {
    ...
    byId.set(n.id, {
      ...
      plan: plan ? { id: plan.id, slug: plan.id.slice(5), title: plan.title, status: plan.status, task: prop(plan.body, 'task') } : undefined,
      requestTask: !!m && plan?.id === `plan:${m[1]}`,
      doc: r ? { project: r.project, slug: r.doc } : null, file: n.file, line: n.line,
      partOf: out.filter(e => e.verb === 'part-of' && !e.to.startsWith('plan:') && !e.to.startsWith('task:')).map(e => e.to),
  ...
  for (const it of byId.values()) {
    ...
    const parentPlan = !it.requestTask ? out.find(e => e.verb === 'part-of' && e.to.startsWith('plan:'))?.to : undefined;
    ...
    const parent = parentTask ? byId.get(parentTask) : parentPlan ? (byId.get(`task:${parentPlan.slice(5)}`) ?? (planNode && byId.get(prop(planNode.body, 'task') ?? ''))) : undefined;
...
export type WorkGroupBy = 'status' | 'goal' | 'plan' | 'document' | 'worker' | 'state';
export type WorkFilter = { q?: string; status?: string; state?: string; worker?: string; goal?: string; plan?: string; doc?: string; done?: boolean; mine?: string };
...
export function groupWork(rows: WorkItem[], by: WorkGroupBy): [string, WorkItem[]][] {
  ...
  const keys = (r: WorkItem): string[] => by === 'status' ? [r.status] : by === 'state' ? [r.state] : by === 'goal' ? (r.partOf.length ? r.partOf : ['— no goal']) : by === 'plan' ? [r.plan?.id ?? '— no plan'] : by === 'document' ? [r.doc ? `${r.doc.project}/${r.doc.slug}` : r.file] : [r.worker ?? '— unassigned'];
...
export function nextReady(rows: WorkItem[], goal?: string): WorkItem | null {
  ...
  const ok = flat.filter(r => r.ready && !r.blocked && !r.worker && r.status !== 'done' && r.state === 'unassigned' && !r.archived && (!goal || r.partOf.includes(goal)) && r.plan?.status !== 'defining');
  return ok[0] ?? null;
}
