...
export function sessionChanges(s: Session, g: GraphData): ChangeGroup[] {
  ...
  const legacy = (s.artifacts?.nodes ?? []).filter(id => !blocks.some(b => b.id === id)).map((id): BlockChange => { const n = byId.get(id); return { id, change: 'changed', doc: n ? docIdOf(g, n.file) ?? '' : '', title: n?.title ?? id, at: s.updatedAt }; });
