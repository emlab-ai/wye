...
// The point where the line joining the two centres leaves box `a`, and the side it leaves through.
export function edgeEnd(a: Box, b: Box): End {
  ...
  if (dx !== 0 && Math.abs(dx) * hh >= Math.abs(dy) * hw) {
    return { x: ax + Math.sign(dx) * hw, y: ay + (dy * hw) / Math.abs(dx), side: dx > 0 ? 'right' : 'left' };
  }
  if (dy !== 0) return { x: ax + (dx * hh) / Math.abs(dy), y: ay + Math.sign(dy) * hh, side: dy > 0 ? 'bottom' : 'top' };
  return { x: ax + hw, y: ay, side: 'right' };   // one card exactly over the other: pick a side and move on
}

// Both ends of one link, each facing the other card.
export function edgeEnds(a: Box, b: Box): { from: End; to: End } { return { from: edgeEnd(a, b), to: edgeEnd(b, a) }; }
