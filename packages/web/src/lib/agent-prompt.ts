...
export async function agentSystemPrompt(product: string, productDir: string, wfUrl: string): Promise<string> {
  ...
  // every project's plan document, so follow-ups have a named home
  const plans: string[] = [];
  try { for (const proj of await readdir(path.join(productDir, 'projects'))) { try { const files = await readdir(path.join(productDir, 'projects', proj, 'docs')); const plan = files.find(f => /plan/i.test(f)); if (plan) plans.push(`- project \`${proj}\`: ${path.join(REPO_ROOT, 'data/products', product, 'projects', proj, 'docs', plan)}`); } catch { /* no docs */ } } } catch { /* no projects */ }
