...
export async function agentSystemPrompt(product: string, productDir: string, wfUrl: string, role: 'worker' | 'librarian' = 'worker'): Promise<string> {
  ...
  const env = `\n\n## This product\n- product: \`${product}\` (WYE_PRODUCT=${product}); Wye repo: ${REPO_ROOT}; app: ${wfUrl}\n- documents: ${REPO_ROOT}/data/products/${product}/projects/<project>/docs/*.md; graph check: \`wye check --root data/products/${product}\` (run from ${REPO_ROOT})\n- the \`wye\` CLI is on PATH (WYE_URL=${wfUrl})${plans.length ? `\n- plan documents (follow-ups go here as task lines):\n${plans.join('\n')}` : ''}`;
