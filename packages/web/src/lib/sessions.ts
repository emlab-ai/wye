...
import { AGENTS, nextTake, type ChatEvent, type QueueItem, type Runner, type Session, type SessionHook, type SessionRole, type SessionSource, type SessionStatus } from './session-types';
...
export async function createSession(productDir: string, product: string, input: { agent: string; instruction: string; refs?: string[]; source?: SessionSource; mode?: 'run' | 'chat'; cwd?: string; images?: { name?: string; dataUrl: string }[]; task?: string; role?: SessionRole; hook?: SessionHook; skills?: string[]; hooks?: string[] }): Promise<Session> {
  const now = new Date().toISOString();
  const s: Session = { id: randomBytes(5).toString('hex'), product, agent: input.agent, mode: input.mode ?? 'run', cwd: input.cwd, ...(input.task ? { task: input.task } : {}), ...(input.role && input.role !== 'worker' ? { role: input.role } : {}), ...(input.hook ? { hook: input.hook } : {}), ...(input.skills?.length ? { skills: input.skills } : {}), ...(input.hooks?.length ? { hooks: input.hooks } : {}), status: 'queued', createdAt: now, updatedAt: now, instruction: input.instruction, refs: [...new Set(input.refs ?? [])], source: input.source ?? {}, log: [{ t: now, line: input.mode === 'chat' ? 'chat session created' : `queued for ${input.agent}` }] };
  const images: string[] = [];
