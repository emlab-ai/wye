...
export function prDocNote(prDoc?: string, s?: Session): string {
  // a task's session has no request page (constraint:wf2.pr-is-the-persons): the task's own document is where it works
  if (!prDoc && s?.task && s.source?.doc) return `\n## Your page\nThis task fills the document \`${s.source.link ?? `${s.product}/${s.source.project}/${s.source.doc}`}\` — the page the task sits on. Write there (wye doc write, wye node set); a task's session has no request page — a PR is only ever a person's request. Tick the task when the page is done: \`wye node set ${s.task} --status done\`.`;
